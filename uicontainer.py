...
class FlowContainer(ui.View):
    ...
    def layout(self):
        ...
        rowheight=currow=w=h=0    # view's width and height, after layout completes'
...
if __name__=="__main__":
    # few examples
    import console, random, string

    # set up a view
    v=ui.View()
    v.add_subview(ui.Switch(frame=(400,5,50,20),name='switch'))
    t=ui.Label(frame=(460,10,200,20))
    t.text='random sizes'
    v.add_subview(t)
    ...
    def addbut(sender,toview):
        ...
        if root['switch'].value:
            ...
            w=h=40
        title=random.choice(string.ascii_lowercase)  # [random.randint(0,26)]
        for v in toview:
